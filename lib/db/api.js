...
//修改用戶登錄密碼
dbapi.saveUserInfo = function(username, password) {
  return db.run(r.table('users').filter({name:username}).update({password:password}))
}
//通过用户名获取用户信息
dbapi.findUserByName = function(username) {
  return db.run(r.table('users').filter({name:username}))
}

dbapi.findAllUser = function() {
