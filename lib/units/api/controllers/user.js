...
function updateUser(req, res) {
  var data = req.swagger.params.user.value
  var userName = data.userName//用戶名
  var oldPassword = data.oldPassword//旧登录密碼
  var newPassword = data.newPassword//新登录密码

  dbapi.findUserByName(userName)
    .then(function(cursor) {
      return Promise.promisify(cursor.toArray, cursor)()
        .then(function(list) {
          list.forEach(function(user) {
            if(user === null) {
              return res.json({success: true, status: 'USER_NULL'})
            }
            if(user.password !== oldPassword) {
              return res.json({success: true, status: 'USER_PASSWORD_ERR'})
            }
            dbapi.saveUserInfo(userName, newPassword)
              .then(function() {
                dbapi.saveUserAfterLogin(user)
                return res.json({success: true, status: 'SUCCESS'})
              })
              .catch(function(err) {
                log.error('Failed to modify login password', err.stack)
                res.status(500).json({
                  success: false
                })
              })
